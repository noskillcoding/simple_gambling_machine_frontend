    ...
    <>
      <div className={`text-center flex-shrink-0 ${marginY}`}>
        <button
          ...
          disabled={isLoading} 
          className={`w-full ${padding} bg-green-500 text-white ${textSize} font-semibold rounded-md hover:bg-green-600 shadow-sm disabled:bg-gray-400 disabled:cursor-not-allowed truncate transition-colors duration-300 ease-in-out`}
        >
          {buttonTextContent}
        </button>
      </div>
